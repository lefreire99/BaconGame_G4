...
public class PrimaryController implements Initializable {
    ...
    private void dijkstra(){
        if(dijkstraBox.getChildren().size()>1){
            ...
            Deque<String> rutaDijkstra = Util.grafo.caminoMinimo(txtActor1.getText(), txtActor2.getText());
            Label numActores1=new Label("Cantidad: "+rutaDijkstra.size());
            while(!rutaDijkstra.isEmpty()){
                System.out.println(rutaDijkstra.pop());
            }
    ...
    private void bfs(){
        if(bfsBox.getChildren().size()>1){
            ...
            Deque<String> rutaBfs = Util.grafo.caminoMinimo(txtActor1.getText(), txtActor2.getText());
            Label numActores2=new Label("Cantidad: "+rutaBfs.size());
            while(!rutaBfs.isEmpty()){
                System.out.println(rutaBfs.pop());
            }
    ...
    private void dfs(){
        if(dfsBox.getChildren().size()>1){
            ...
            Deque<String> rutaDfs = Util.grafo.caminoMinimo(txtActor1.getText(), txtActor2.getText());
            Label numActores3=new Label("Cantidad: "+rutaDfs.size());
            while(!rutaDfs.isEmpty()){
                System.out.println(rutaDfs.pop());
            }
